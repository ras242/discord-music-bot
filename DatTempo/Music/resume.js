...
execute(message) {
    ...
    if (!queue.playing) {
      ...
      .setAuthor(`${message.author.username} resumed the music!`, "https://i.imgur.com/fN8WD7A.gif")
      //send the approve
      return queue.textChannel.send(playembed).catch(console.error);
    }
    //if its not paused return error
    return  attentionembed(message, "The Queue is not paused <:StingyB:899626440524447794>!").catch(console.error);
  }
};
