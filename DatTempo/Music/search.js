...
async execute(message,args,client) {
    ...
    if (message.channel.activeCollector)
      return attentionembed(message,"There is a search active <:Monkey:881888675082948688>!");
    //if the user is not in a voice channel return error
    if (!message.member.voice.channel)
      return attentionembed(message,"Please join a Voice Channel first <:StingyB:899626440524447794>")
       //If not in the same channel return error
    if (serverQueue && channel !== message.guild.me.voice.channel)
    return attentionembed(message, `You must be in the same Voice Channel as me <:StingyB:899626440524447794>`);
    //define search
    const search = args.join(" ");
    //define a temporary Loading Embed
    let temEmbed = new MessageEmbed()
    .setAuthor("Searching...", "https://i.imgur.com/fN8WD7A.gif")
    .setColor("#F0EAD6")
    ...
    let resultsEmbed = new MessageEmbed()
      ...
      .setColor("#F0EAD6")
      .setAuthor("Search results <:Funny:899620543706300429>!!", "https://i.imgur.com/fN8WD7A.gif","http://harmonymusic.tk")
      .setFooter("Response with your favorite number","https://i.imgur.com/fN8WD7A.gif")
