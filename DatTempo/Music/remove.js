...
execute(message, args) {
  //if its not a guild return
    ...
    if (!args.length) return attentionembed(message,`Try: ${message.client.prefix}remove <Queue Number> <:Monkey:881888675082948688>`);
    //If not a number then return error
    if (isNaN(args[0])) return attentionembed(message,`Try: ${message.client.prefix}remove <Queue Number> <:Monkey:881888675082948688>`);
    //get the song
    ...
    .setDescription(`:no_entry_sign: | ${message.author} removed **${song[0].title}** from the Queue <:StingyB:899626440524447794>`)
    .setColor("#F0EAD6")
    );
  }
};
